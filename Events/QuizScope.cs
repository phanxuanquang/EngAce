...
namespace Events
{
    public static class QuizScope
    {
        ...
        public static async Task<List<Quiz>?> GenerateQuizes(string apiKey, string topic, List<QuizzType> quizzTypes, EnglishLevel level, short questionsCount)
        {
            ...
            promptBuilder.Append("Nội dung của bộ đề phải là những thông tin chính xác và đã được kiểm duyệt, mỗi câu hỏi trong bộ đề trắc nghiệm chỉ được phép có 4 lựa chọn với duy nhất 1 lựa chọn đúng.");
